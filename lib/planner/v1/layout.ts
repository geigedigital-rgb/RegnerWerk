...
function flowFor(
  ...
  const opt =
    emitters.rotor.options.find((o) => o.radiusM >= radiusM) ??
    emitters.rotor.options[emitters.rotor.options.length - 1];
  return opt?.flowLMin ?? 10;
}

function rotorNozzleFor(emitters: BrandEmitters, radiusM: number): string {
  const opt =
    emitters.rotor.options.find((o) => o.radiusM >= radiusM) ??
    emitters.rotor.options[emitters.rotor.options.length - 1];
...
/**
 * Head-to-head radius: prefer full safe clearance (designer coverage),
 * clamped to manufacturer min–max — not shrunk down to neighbour distance.
 */
function resolveRadius(
  ...
  // Stay just inside the hard boundary
  const maxSafe = Math.max(0, clearance * 0.995);

  let desired: number;
  if (maxSafe >= fam.radiusMinM) {
    // Use available throw — larger radius = fewer dry pockets
    desired = Math.min(fam.radiusMaxM, maxSafe);
  } else {
    // Manufacturer floor > safe clearance → must use min, overshoot risk
    desired = fam.radiusMinM;
  }

  const radiusM = Number(
    Math.min(fam.radiusMaxM, Math.max(fam.radiusMinM, desired)).toFixed(1),
  );
  return {
    radiusM,
...
function layoutStripZone(
  ...
  const spacing = Math.max(spec.lengthM * 0.85, widthM + 0.2);
...
function finalizeHeads(
  ...
  return kept.map((h, idx) => {
    ...
    const configKey =
      fam.kind === "spray"
        ? isFull
          ? `${fam.familyKey}-360`
          : fam.familyKey
        : `${rotorPrefix}@${rotorNozzleFor(emitters, radiusM)}`;
    ...
    return {
      ...
      flowLMin: Number(
        flowFor(emitters, fam.kind, fam.familyKey, radiusM, arc).toFixed(2),
      ),
      lawnZoneId: zoneId,
    };
  });
...
export function layoutLawnZone(
  ...
  // with head-to-head along the perimeter. Prefer slightly larger spacing —
  // radii later expand to full clearance, so fewer heads still cover.
  ...
  // Designer spacing: ~1.0–1.1× working R (was 0.95 → too dense)
  const S = R * 1.05;
  ...
  /** Block only if another head is physically on top or a 360 already covers here. */
  function blocksInterior(pt: PtM): boolean {
    // Full-circle heads belong in the open lawn — not on the boundary
    if (distToBoundary(pt, ring) < Math.max(1.6, smallMin * 0.65)) return true;
    for (const h of kept) {
      const d = dist(h.pt, pt);
      if (d < Math.max(1.6, S * 0.55)) return true;
  ...
  while (dry.length > 0 && added < maxInterior) {
    ...
    for (const p of dry) {
      if (obstacleRings.some((o) => pointInPolygon(p, o))) continue;
      if (blocksInterior(p)) continue;

      const clearance = sectorClearanceM(p, 0, 360, ring, obstacleRings);
      // Need real interior room for a 360°
      if (clearance < smallMin * 1.05) continue;
      if (distToBoundary(p, ring) < Math.max(1.6, smallMin * 0.65)) continue;
  ...
  // ——— 5. Prune redundant heads (edges / interior) if coverage stays ≥ 97 % ———
  // Keep true lawn corners (priority 0); drop surplus façade/edge/interior.
  if (kept.length > 4) {
    for (let i = kept.length - 1; i >= 0; i--) {
      ...
      kept.length = 0;
      kept.push(...saved);
      if (total > 0 && dryWithout.length / total <= 0.03) {
        kept.splice(i, 1);
      }
    }
  }
