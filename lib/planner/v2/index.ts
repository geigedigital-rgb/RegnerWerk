...
export type { SofortPlanV2 } from "./types";

const ALGORITHM_BUILD = "v2.stage3-2026.08";

/**
 * Sofort-Berechnung v2 — iterative engineering pipeline per spec.
 * Returns UI view model via adapter; use computeSofortPlanV2Raw for full model.
 */
export function computeSofortPlanV2(
  zones: DrawnZone[],
  fixtures: PlotFixture[],
  opts?: { brand?: SprinklerBrand },
): SofortPlan {
  return adaptV2ToViewModel(computeSofortPlanV2Raw(zones, fixtures, opts));
}

export function computeSofortPlanV2Raw(
  zones: DrawnZone[],
  fixtures: PlotFixture[],
  opts?: { brand?: SprinklerBrand },
): SofortPlanV2 {
  ...
  const cap = source.flowLMin * fill;
...
/** Recompute after drag/delete — reuse v2 zoning + pipe sizing. */
export function recomputeAfterEditV2(
  plan: SofortPlan,
  fixtures: PlotFixture[],
  zones: DrawnZone[],
): SofortPlan {
  ...
  const verteilerPos = verteiler?.position ?? quelle?.position ?? null;
  const source = extractSourceCurve(fixtures);
  const cap = source.flowLMin * CATALOG.hydraulics.zoneFillFactor;
  const sourcePressure =
    source.dynamicPressureBar ??
    CATALOG.hydraulics.assumedVerteilerPressureBar;
