...
export function buildBom(params: {
  ...
  if (byOd.size === 0) {
    ...
    for (const [od, lenM] of byOd) {
      ...
      const picked = pickRolls(Math.ceil(lenM), rolls);
      let rollQty = 0;
      for (const { roll, qty } of picked) {
        rollQty += qty;
        bom.push(
          line({
      ...
      if (coupling && rollQty > 1) {
        bom.push(
          line({
            key: `pe-coupling-${od}`,
            article: coupling.article,
            label: shortGeneric(coupling.label, 48),
            qty: rollQty - 1,
            unit: "piece",
            priceEur: coupling.priceEur,
            group: "rohr",
            note: `Rollenstöße PE ${od}`,
            imageUrl: partImage(coupling),
          }),
        );
      }
    }
  ...
  for (const [od, qty] of [...elbowCounts.entries()].sort((a, b) => a[0] - b[0])) {
    if (qty <= 0) continue;
    const elbow = peElbowPart(od);
    if (!elbow) continue;
    bom.push(
      line({
        key: `pe-elbow-${od}`,
        article: elbow.article,
        label: shortGeneric(elbow.label, 48),
        qty,
        unit: "piece",
        priceEur: elbow.priceEur,
        group: "rohr",
        note: `90°-Knicke auf PE-${od}-Trasse (laut Leitungsweg)`,
        imageUrl: partImage(elbow),
      }),
    );
  }
